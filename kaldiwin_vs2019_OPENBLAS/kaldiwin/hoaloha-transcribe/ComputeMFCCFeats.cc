...
int ComputeMFCCFeats(MfccOptions mfcc_opts, std::string wav_file_path, Matrix<BaseFloat>* pfeatures) {
    try {
        ...
        if (wave_data.Duration() < min_duration) {
            KALDI_WARN << "File: " << wav_file_path << " is too short ("
        ...
        {  // This block works out the channel (0=left, 1=right...)
            ...
            if (channel == -1) {
                ...
                if (this_chan >= num_chan) {
                    KALDI_WARN << "File" << wav_file_path << " has "
        ...
        try {
            ...
            KALDI_WARN << "Failed to compute features for utterance " << wav_file_path;
        }
        if (subtract_mean) {
