...
namespace hoalohadll
{
	...
	extern "C" HOALOHA_TRANSCRIBE_API int TranscribeFile(intptr_t model, CHAR * wav_file_path, LPEXTFUNCWRITE transcription, double& likelihood);
	extern "C" HOALOHA_TRANSCRIBE_API int TranscribeWaveData(intptr_t model, intptr_t data, int samp_freq, int sample_count, LPEXTFUNCWRITE write, double& likelihood);
